...
namespace sms
{
    public partial class _Default : System.Web.UI.Page
    {
        ...
        string logged = Session["LoggedIn"] as string;
        bool loggedIn;
        bool.TryParse(logged, out loggedIn);
        if (!loggedIn)
        {
        ...
        protected void ApproveStudent_OnClick(object sender, EventArgs e)
        {
        Label id = GridView1.Rows[e.RowIndex].FindControl("txtID") as Label;
            ...
            string css = ConfigurationManager.ConnectionStrings["ApplicationServices"].ToString();
            SqlConnection ocon = new SqlConnection(css);
            SqlCommand cmd;
            ocon.Open();
            ...
            //updating the record  
                ...
                cmd = new SqlCommand("Update STUDENT set [Approved] = 'Pending' where STD_ID=" +Convert.ToInt32( id.Text), ocon);
                cmd.ExecuteNonQuery();
                ocon.Close();
       ...
       SqlConnection ocon = new SqlConnection(css);     
       ocon.Open();
       adapt = new SqlDataAdapter("Select * from STUDENT where [Approved] = 'Pending'", ocon);  
        adapt.Fill(dt);  
        if(dt.Rows.Count>0)  
        {  
      ...
      //      string query1 = "insert into STUDENT(STD_ID,STD_NAME,STD_FNAME,STD_ADDRESS,STD_TEL) values ("+ TextBox1.Text+ "," + TextBox2.Text + "," + TextBox3.Text + "," + TextBox4.Text + "," + TextBox5.Text + ")";
            ...
            cmd = new SqlCommand(query1, ocon);
            cmd.ExecuteNonQuery();
            ocon.Close();
            TextBox1.Text = "";
            TextBox2.Text = "";
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            //updating the record  
                ...
                cmd = new SqlCommand("Update STUDENT set STD_NAME='" + name.Text + "',STD_FNAME='" + fname.Text + "',STD_ADDRESS='" + address.Text + "',STD_TEL='" + tel.Text + "' where STD_ID=" +Convert.ToInt32( id.Text), ocon);
                cmd.ExecuteNonQuery();
                ocon.Close();
                //Setting the EditIndex property to -1 to cancel the Edit mode in Gridview  
                GridView1.EditIndex = -1;
        ...
        protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            cmd = new SqlCommand("Delete From STUDENT where STD_ID=" +int.Parse(id.Text), ocon);
            cmd.ExecuteNonQuery();
            ocon.Close();
            //Setting the EditIndex property to -1 to cancel the Edit mode in Gridview  
            GridView1.EditIndex = -1;
